    ...
    def patch_wrapper(request, patch_filename, *args, **kwds):
        ...
        if patch is None and patch_filename.isdigit():
            ...
            patch = models.Patch.get_by_id(
                int(patch_filename), parent=request.patchset.key
            )
        if patch is None:
            return respond(
                request,
    ...
    def decorator(func):
        ...
        def wrapped(request, *args, **kwds):
            if request.method not in methods:
                ...
                rsp = HttpTextResponse(
                    "This requires a specific method: %s" % allowed, status=405
                )
                rsp["Allow"] = allowed
                return rsp
            return func(request, *args, **kwds)
    ...
    def user_key_wrapper(request, user_key, *args, **kwds):
        ...
        if "@" in user_key:
            ...
            if not account:
                ...
                return HttpTextResponse(
                    "No user found with that key (%s)" % urllib.parse.quote(user_key),
                    status=404,
                )
    ...
    def xsrf_wrapper(request, *args, **kwds):
        if request.method == "POST":
            ...
            if post_token != xsrf_token:
                ...
                if post_token != xsrf_token:
                    msg = ["Invalid XSRF token."]
                    if request.POST:
                        msg.extend(
                            ["", "However, this was the data posted to the server:", ""]
                        )
                        for key in request.POST:
                            msg.append("%s: %s" % (key, request.POST[key]))
                        msg.extend(
                            [
                                "",
                                "-" * 10,
