...
def get_links_for_users(user_emails):
    ...
    memcache_results = memcache.get_multi(remaining_emails, key_prefix="show_user:")
...
def show_reviewers(reviewer_list, arg=None):
    ...
    email_list = []
    for reviewer, _approval in list(reviewer_list.items()):
        email = reviewer
        if isinstance(email, users.User):
            email = email.email()
    ...
    return django.utils.safestring.mark_safe(
        ", ".join(
            format_approval_text(links[r], a) for r, a in list(reviewer_list.items())
        )
    )
...
def show_users(email_list, arg=None):
    ...
    return django.utils.safestring.mark_safe(
        ", ".join(links[email] for email in email_list)
    )
