...
class Issue(ndb.Model):
  ...
  """
    ...
    owner = auth_utils.AnyAuthUserProperty(auto_current_user_add=True, required=True)
    ...
    def user_can_view(self, user):
        ...
        return self.user_can_edit(user) or email in self.cc or email in self.reviewers
    ...
    """
        ...
        self.num_messages = 0
        old_messages = Message.query(Message.draft == False, ancestor=self.key).order(
            Message.date
        )
        for msg in itertools.chain(old_messages, msgs):
            ...
            if msg.sender == self.owner.email():
                updates_for_set.update(
                    self.reviewers, self.cc, self.collaborator_emails()
                )
            else:
    ...
    for other in patchsets:
        ...
        if other.data or other.parsed_patches:
            ...
            if other_patches and len(other_patches) > 1:
                logging.info(
                    "Got %s patches with the same filename for a patchset",
                    len(other_patches),
                )
...
class Message(ndb.Model):
  ...
  """
    ...
    """
        ...
        return any(
            True
            for line in self.text.lower().splitlines()
    ...
    """
        ...
        if rev is not None:
            if rev == 0:
                ...
                return Content(text="", parent=self.key)
  ...
  """
    ...
    def complete(self):
        ...
        def _Append():
            if cur_bucket:
                self.buckets.append(
                    Bucket(text="\n".join(cur_bucket), quoted=bool(quoted))
                )
...
class Account(ndb.Model):
  ...
  """
    ...
    user = auth_utils.AnyAuthUserProperty(auto_current_user_add=True, required=True)
    email = ndb.StringProperty(required=True)  # key == <email>
    nickname = ndb.StringProperty(required=True)
    default_context = ndb.IntegerProperty(
        default=settings.DEFAULT_CONTEXT, choices=CONTEXT_CHOICES
    )
    default_column_width = ndb.IntegerProperty(default=settings.DEFAULT_COLUMN_WIDTH)
    created = ndb.DateTimeProperty(auto_now_add=True)
    modified = ndb.DateTimeProperty(auto_now=True)
    ...
    def create_nickname_for_user(cls, user):
        ...
        existing_nicks = [
            account.lower_nickname
            for account in cls.query(
                cls.lower_nickname >= nickname.lower(), cls.lower_nickname < next_char
            )
        ]
        suffix = 0
    ...
    """
        ...
        draft_query = Comment.query(Comment.author == self.user, Comment.draft == True)
        issue_ids = set(
            comment.key.parent().parent().parent().id() for comment in draft_query
        )
    ...
    def get_xsrf_token(self, offset=0):
        ...
        if self.user.user_id() != current_user.user_id():
            ...
            logging.info(
                "Updating user_id for %s from %s to %s"
...
class AccountStatsBase(ndb.Model):
  ...
  """
    ...
    #                 sent.
    NORMAL, IGNORED, DRIVE_BY, NOT_REQUESTED, OUTGOING = list(range(1, 6))
    REVIEW_TYPES = [
        None,
        "normal",
        "ignored",
    ...
    def median_latency(self):
        ...
        if (length & 1) == 0:
            return (latencies[length / 2] + latencies[length / 2 - 1]) / 2.0
        else:
    ...
    def percent_reviewed(self):
        ...
        if not self.nb_issues:
            return 0
        return self.nb_reviewed * 100.0 / self.nb_issues
    ...
    def percent_lgtm(self):
        ...
        if not self.nb_issues:
            return 0
        return self.nb_lgtmed * 100.0 / self.nb_issues
    ...
    def _pre_put_hook(self):
        ...
        assert (
            ...
            == len(self.lgtms)
            == len(self.latencies)
            == len(self.review_types)
        ), str(self)
        assert all(i >= 0 for i in self.lgtms), str(self)
        assert all(i >= -1 for i in self.latencies), str(self)
        assert all(self.NORMAL <= i <= self.OUTGOING for i in self.review_types), str(
            self
        )
        for i in range(len(self.issues)):
        ...
        if not self._days:
            if self.name.isdigit():
                ...
                if not quarter:
                    ...
                    self._days = sum(
                        calendar.monthrange(list(map(int, i.split("-"))))[1]
                        for i in quarter
                    )
        return self._days
...
def sum_account_statistics(out, items):
  ...
  """
    ...
    return (
        prev_issues != out.issues
        or prev_latencies != out.latencies
        or prev_lgtms != out.lgtms
        or prev_review_types != out.review_types
    )
