...
def ParsePatchToChunks(lines, name="<patch>"):
  ...
  """
        ...
        if match:
            if raw_chunk:
                ...
                if len(old_chunk) != old_j - old_i or len(new_chunk) != new_j - new_i:
                    logging.warn(
                        "%s:%s: previous chunk has incorrect length", name, lineno
                    )
                    return None
            ...
            old_ln, old_n, new_ln, new_n = list(
                map(int, (old_ln, old_n or 1, new_ln, new_n or 1))
            )
            ...
            if old_i < old_last or new_i < new_last:
                logging.warn("%s:%s: chunk header out of order: %r", name, lineno, line)
                return None
            if old_i - old_last != new_i - new_last:
                logging.warn("%s:%s: inconsistent chunk header: %r", name, lineno, line)
                return None
            old_last = old_j
            new_last = new_j
        else:
    ...
    if raw_chunk:
        ...
        if len(old_chunk) != old_j - old_i or len(new_chunk) != new_j - new_i:
            print(
                ("%s:%s: last chunk has incorrect length" % (name, lineno)),
                file=sys.stderr,
            )
            return None
        chunks.append((old_range, new_range, old_chunk, new_chunk))
