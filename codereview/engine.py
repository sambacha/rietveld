...
def RenderDiff2TableRows(
  ...
  Yields:
    ...
    Each yielded string may consist of several <tr> elements.
  """
    rows = _RenderDiff2TableRows(
        request, old_lines, old_patch, new_lines, new_patch, colwidth, debug
    )
    return _CleanupTableRowsGenerator(rows, context)
...
def _ShortenBuffer(buf, context):
  ...
  """
    if context is None or len(buf) < 3 * context:
        ...
        yield (
            '<tr id="skip-%d"><td colspan="2" align="center" '
    ...
    for patch, dct in [(old_patch, old_dict), (new_patch, new_dict)]:
        if patch:  # Skip if the patchset had no patch for this file.
            query = models.Comment.query(
                models.Comment.patch_key == patch.key, models.Comment.left == False
...
def _GetComments(request):
  ...
  """
    old_dict = {}
    new_dict = {}
    query = models.Comment.query(models.Comment.patch_key == request.patch.key).order(
        models.Comment.date
    )
    for comment in query:
        if comment.draft and comment.author != request.user:
...
def _TableRowGenerator(
  ...
  """
    ...
    if old_patch == new_patch and (old_max == 0 or new_max == 0):
        ...
        yield "", (
            '<tr><td class="info" colspan="2">' "(Both sides are equal)</td></tr>"
        )
    ...
    for tag, old, new in triple_iterator:
        ...
        for i in range(max(len(old), len(new))):
            ...
            if do_ir_diff:
                ...
                for tg, frag in _RenderDiffInternal(
                    old_buff_out,
                    new_buff_out,
                    ndigits,
                    tag,
...
def _RenderDiffInternal(
    ...
    obegin = (
        intra_region_diff.BEGIN_TAG % intra_region_diff.COLOR_SCHEME["old"]["match"]
    )
    nbegin = (
        intra_region_diff.BEGIN_TAG % intra_region_diff.COLOR_SCHEME["new"]["match"]
    )
    oend = intra_region_diff.END_TAG
    nend = oend
    user = auth_utils.get_current_user()

    for i in range(len(old_buff)):
        tg = tag
...
def _RenderDiffColumn(
  ...
  """
    if line_valid:
        ...
        return '<td class="%s" id="%scode%d">%s</td>' % (
            cls_attr,
            prefix,
            lineno,
            col_content,
...
def _RenderInlineComments(
    line_valid, lineno, data, user, patch, snapshot, prefix, request
):
    """Helper function for _RenderDiffInternal().
...
def _ExpandTemplate(name, request, **params):
  ...
  """
    rslt = loader.render_to_string(
        name, params, context_instance=RequestContext(request)
    )
