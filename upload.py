...
VCS = [
    ...
    {"name": VCS_SUBVERSION, "aliases": ["svn", "subversion"],},
    {"name": VCS_PERFORCE, "aliases": ["p4", "perforce"],},
    {"name": VCS_GIT, "aliases": ["git"],},
    {"name": VCS_CVS, "aliases": ["cvs"],},
]
...
class AbstractRpcServer(object):
        ...
        try:
            ...
            if e.code == 403:
                ...
                raise ClientLoginError(
                    req.get_full_url(), e.code, e.msg, e.headers, response_dict
                )
            else:
                raise
    ...
    """
        ...
        req = self._CreateRequest(
            "%s/_ah/login?%s" % (self.host, urllib.parse.urlencode(args))
        )
        try:
            response = self.opener.open(req)
        except urllib.error.HTTPError as e:
        ...
        if self.save_cookies:
            ...
            if os.path.exists(self.cookie_file):
                try:
                    ...
                    self.authenticated = True
                    StatusUpdate(
                        "Loaded authentication cookies from %s" % self.cookie_file
                    )
...
group.add_option(
    "--noisy", action="store_const", const=3, dest="verbose", help="Print all logs."
)
group.add_option(
    "--print_diffs", dest="print_diffs", action="store_true", help="Print full diffs."
)
# Review server
...
group.add_option(
    ...
    default=True,
    dest="use_oauth2",
    help=("Use password instead of OAuth 2.0. " "Useful with development server."),
)
group.add_option(
    "--oauth2_port",
    action="store",
    type="int",
    dest="oauth2_port",
    default=DEFAULT_OAUTH2_PORT,
    help=(
        "Port to use to handle OAuth 2.0 redirect. Must be an "
...
group.add_option(
    "--no_oauth2_webbrowser",
    action="store_false",
    dest="open_oauth2_local_webbrowser",
    default=True,
...
group.add_option(
    "-j",
    "--number-parallel-uploads",
    dest="num_upload_threads",
    default=8,
...
group.add_option(
    ...
    dest="file",
    default=None,
    help="Read the message above from file.",
)
group.add_option(
    "-r",
    "--reviewers",
...
group.add_option(
    ...
    default=None,
    help=(
        "Explicitly specify version control system (%s)" % ", ".join(VCS_SHORT_NAMES)
    ),
)
group.add_option(
...
group.add_option(
    ...
    default=50,
    help=(
        "Set the minimum similarity percentage for detecting "
        "renames and copies. See `git diff -C`. (default 50)."
    ),
)
group.add_option(
    "--git_only_search_patch",
    action="store_false",
    default=True,
...
def OpenOAuth2ConsentPage(server=DEFAULT_REVIEW_SERVER, port=DEFAULT_OAUTH2_PORT):
...
def GetAccessToken(
    server=DEFAULT_REVIEW_SERVER, port=DEFAULT_OAUTH2_PORT, open_local_webbrowser=True
):
...
def RunShellWithReturnCodeAndStderr(
    command, print_output=False, universal_newlines=True, env=os.environ
):
...
def RunShellWithReturnCode(
    ...
    out, err, retcode = RunShellWithReturnCodeAndStderr(
        command, print_output, universal_newlines, env
    )
    return out, retcode
    ...
    def UploadBaseFiles(self, issue, rpc_server, patch_list, patchset, options, files):
        ...
        def UploadFile(filename, file_id, content, is_binary, status, is_base):
            ...
            ctype, body = EncodeMultipartFormData(
                form_fields, [("data", filename, content)]
            )
            try:
        ...
        for filename in list(patches.keys()):
            ...
            if base_content != None:
                t = thread_pool.apply_async(
                    UploadFile,
                    args=(filename, file_id, base_content, is_binary, status, True),
                )
                threads.append(t)
            if new_content != None:
                t = thread_pool.apply_async(
                    UploadFile,
                    args=(filename, file_id, new_content, is_binary, status, False),
                )
                threads.append(t)
...
class SubversionVCS(VersionControlSystem):
    ...
    def GetStatus(self, filename):
        ...
        if not self.options.revision:
            status = RunShell(
                ["svn", "status", "--ignore-externals", self._EscapeFilename(filename)]
            )
            if not status:
            ...
            if (
                len(status_lines) in (3, 4)
                and not status_lines[0]
                and status_lines[1].startswith("--- Changelist")
            ):
        ...
        # the correct status for a file.
            ...
            if dirname not in self.svnls_cache:
                ...
                if returncode:
                    ...
                    if re.match(
                        "^svn:( E195012:)? Unable to find repository location for .+ in revision \d+",
                        err,
                    ):
                        old_files = ()
                    else:
    ...
    def GetBaseFile(self, filename):
        ...
        if status[0] == "A" and status[3] != "+":
            ...
            mimetype = RunShell(
                ["svn", "propget", "svn:mime-type", self._EscapeFilename(filename)],
                silent_ok=True,
            )
            base_content = ""
            ...
            status[0] in ("M", "D", "R")
            or (status[0] == "A" and status[3] == "+")
            or (status[0] == " " and status[1] == "M")  # Copied file.
        ):  # Property change.
            args = []
            if self.options.revision:
            ...
            is_binary = (
                ...
                and mimetype not in ("image/x-xbitmap", "image/x-xpixmap")
            )
            if status[0] == " ":
                ...
                if status[0] == "M":
                    if not self.rev_end:
                        ...
                        new_content = RunShell(
                            ["svn", "cat", url], universal_newlines=True, silent_ok=True
                        )
            else:
                get_base = True
...
class GitVCS(VersionControlSystem):
    ...
    def GetUnknownFiles(self):
        status = RunShell(
            ["git", "ls-files", "--exclude-standard", "--others"], silent_ok=True
        )
    ...
    def GetFileContent(self, file_hash):
        ...
        data, retcode = RunShellWithReturnCode(
            ["git", "show", file_hash], universal_newlines=False
        )
        if retcode:
...
class MercurialVCS(VersionControlSystem):
    ...
    def GetUnknownFiles(self):
        ...
        args = []
        status = RunShell(
            ["hg", "status", "-S", "--rev", self.base_rev, "-u", "."], silent_ok=True
        )
        unknown_files = []
    ...
    def GetBaseFile(self, filename):
        ...
        if status != "A":
            base_content = RunShell(
                ["hg", "cat", "-r", base_rev, oldrelpath], silent_ok=True
            )
...
class PerforceVCS(VersionControlSystem):
    ...
    def __init__(self, options):
        ...
        if not options.title:
            description = self.RunPerforceCommand(
                ["describe", self.p4_changelist], marshal_output=True
            )
    ...
    def RunPerforceCommandWithReturnCode(
        self, extra_args, marshal_output=False, universal_newlines=True
    ):
        args = ["p4"]
        if marshal_output:
    ...
    def GetFileContent(self, filename, revision, is_binary):
        ...
        return self.RunPerforceCommand(
            ["print", "-q", file_arg], universal_newlines=not is_binary
        )
    ...
    def GetBaseFilename(self, filename):
        ...
        if self.GetAction(filename) in actionsWithDifferentBases:
            ...
            fstat_result = self.RunPerforceCommand(
                ["fstat", "-Or", filename], marshal_output=True
            )
    ...
    def GetBaseRevision(self, filename):
        ...
        have_result = self.RunPerforceCommand(
            ["have", base_filename], marshal_output=True
        )
    ...
    def GenerateDiff(self, args):
        ...
        def GenerateMergeDiff(diffData, args):
            ...
            first_good_line = 0
            while first_good_line < len(lines) and not lines[
                first_good_line
            ].startswith("@@"):
                first_good_line += 1
        ...
        def GenerateAddDiff(diffData):
            fstat = self.RunPerforceCommand(
                ["fstat", diffData.filename], marshal_output=True
            )
            if "headRev" in fstat:
        ...
        def GenerateDeleteDiff(diffData):
            ...
            diffData.file_body = self.GetFileContent(
                diffData.base_filename, None, is_base_binary
            )
            # Replicate svn's list of changed lines
            line_count = len(diffData.file_body.splitlines())
        ...
        svndiff = []
        filecount = 0
        for (filename, action) in list(changed_files.items()):
            ...
            if svn_status == "M":
                ...
                ErrorExit(
                    "Unknown file action %s (svn action %s)." % (action, svn_status)
                )
    ...
    def GetBaseFile(self, filename):
        ...
        if status != "A":
            ...
            base_content = self.GetFileContent(base_filename, revision, is_base_binary)
...
def GuessVCSName(options):
  ...
  """
    ...
    res = RunDetectCommand(VCS_GIT, ["git", "rev-parse", "--is-inside-work-tree"])
    if res != None:
        return res
...
def LoadSubversionAutoProperties():
  ...
  """
    ...
    if (
        config.has_section("miscellany")
        and config.has_option("miscellany", "enable-auto-props")
        and config.getboolean("miscellany", "enable-auto-props")
        and config.has_section("auto-props")
    ):
        props = {}
  ...
  """
    ...
    if isinstance(vcs, SubversionVCS):
        ...
        if base:
            if guessed_base and base != guessed_base:
                print(
                    'Using base URL "%s" from --base_url instead of "%s"'
                    % (base, guessed_base)
                )
        else:
            base = guessed_base
    ...
    if base:
        ...
        if username:
            ...
            base = urllib.parse.urlunparse(
                (b.scheme, netloc, b.path, b.params, b.query, b.fragment)
            )
    ...
    if not options.download_base or not uploaded_diff_file:
        ...
        msg = response_body
    StatusUpdate(msg)
    if not response_body.startswith("Issue created.") and not response_body.startswith(
        "Issue updated."
    ):
        sys.exit(0)
    ...
    rpc_server.Send(
        "/" + issue_id + "/upload_complete/" + (patchset or ""), payload=payload
    )
