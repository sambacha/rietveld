...
class TestPublish(TestCase):
    ...
    def test_draft_details_no_base_file(self):
        ...
        cmt1 = models.Comment(patch_key=self.patches[0].key, parent=self.patches[0].key)
        ...
        cmt2 = models.Comment(patch_key=self.patches[1].key, parent=self.patches[1].key)
...
class TestSearch(TestCase):
    def setUp(self):
        ...
        issue1.reviewers = ["test@groups.example.com", "bar@example.com"]
        issue1.local_base = False
        issue1.put()
        issue2 = models.Issue(subject="test")
        issue2.reviewers = ["test2@groups.example.com", "bar@example.com"]
    ...
    def test_json_get_api(self):
        ...
        response = self.client.get(
            ...
            {
                "closed": 3,
                "reviewer": "test@groups.example.com",
                "private": 1,
                "created_before": str(end),
...
class TestModifierCount(TestCase):
    ...
    def makePatch(self, add_lines, remove_lines):
        ...
        ) % ((self.line_count(add_lines), self.line_count(remove_lines)))
