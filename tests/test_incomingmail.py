...
class TestIncomingMail(TestCase):
    ...
    def test_incoming_mail(self):
        ...
        response = self.client.post(
            "/_ah/mail/reply@example.com", msg.as_string(), content_type="text/plain"
        )
        ...
        self.assertEqual(msg.subject, "subject (issue%s)" % self.issue.key.id())
        self.assertEqual(msg.sender, "sender@example.com")
        self.assertEqual(msg.recipients, ["reply@example.com"])
        self.assertTrue(msg.date is not None)
    ...
    def test_incoming_mail_invalid_subject(self):
        ...
        msg.set_payload("body")
        response = self.client.post(
            "/_ah/mail/reply@example.com", msg, content_type="text/plain"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_unknown_issue(self):
        ...
        msg.set_payload("body")
        self.assertRaises(
            views.InvalidIncomingEmailError,
    ...
    def test_huge_body_is_truncated(self):  # see issue325
        ...
        self.assertTrue(imsg.text.endswith("... (message truncated)"))
    ...
    def test_missing_encoding(self):
        ...
        imsg = models.Message.query(ancestor=self.issue.key).get()
        self.assertEqual(imsg.text, "Âfoo")
        imsg.key.delete()
        body = "\xf6"
        msg = (
            "From: sender@example.com",
