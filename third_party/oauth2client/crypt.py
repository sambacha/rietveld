...
if OpenSSLSigner:
    ...
    raise ImportError(
        "No encryption library found. Please install either "
  ...
  """
    ...
    if now < earliest:
        raise AppIdentityError(
            "Token used too early, %d < %d: %s" % (now, earliest, json_body)
        )
    if now > latest:
        raise AppIdentityError(
            "Token used too late, %d > %d: %s" % (now, latest, json_body)
        )
    ...
    if audience is not None:
        ...
        if aud != audience:
            raise AppIdentityError(
                "Wrong recipient, %s != %s: %s" % (aud, audience, json_body)
            )
